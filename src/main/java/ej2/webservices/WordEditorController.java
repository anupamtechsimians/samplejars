...
public class WordEditorController {
	...
	@CrossOrigin(origins = "*", allowedHeaders = "*")
	@PostMapping("/api/wordeditor/Load")
	public String loadFile(@RequestBody String s3Link){

		    String accessKey = "";
		    String secretKey = "";
		    String bucketName = "";
		    String objectKey = "document name";
	        String sfdtContent = "";
//
...
//
	            ...
	            sfdtContent = WordProcessorHelper.load(inputStream, com.syncfusion.ej2.wordprocessor.FormatType.Docx);
	        } catch (Exception e) {
	...
	@CrossOrigin(origins = "*", allowedHeaders = "*")
	@PostMapping("/api/wordeditor/BlobExport")
	public String blobExport(@RequestParam("files") MultipartFile file) throws Exception {
		try {
			String name = file.getOriginalFilename();
			String format = retrieveFileType(name);
			WordDocument document = new WordDocument(file.getInputStream());
			...
			e.printStackTrace();
			return "Document export failed due to " + e.getMessage();
		}
	}
